...
def predict():
    ...
    new_df = pd.Dataframe(data,columns = ['SeniorCtizen', 'MonthlyCharges', 'TotalCharges','gender', 'Partner','Depemdents','PhoneService','MultipleLines',
                                          'InternetService','OnlineSecurity','OnlineBackup','DeviceProtection','TechSupport','StreamingTV','StreamingMovies'
                                          ,'Contract','PaperlessBilling','PaymentMethod','tenure'])
    
    df_2 = pd.concat([df_1,new_df],ignore_index=True)
    #Label the tenure in bins of 12 months
